
#define BLOCK_SIZE (128 / 8)

__constant uchar sbox[256] = {
   
};

//nonce added to the counter in ctr mode
__constant uchar nonce[8] = {
  0xf1, 0x42, 0x61, 0xbb, 0xfc, 0x34, 0xc5, 0xe9
};

void subBytes(__private uchar* s){

  #pragma unroll
  for (int i = 0; i < 4*Nb; ++i) {
    s[i] = sbox[s[i]];
  }
  /*s[0] = sbox[s[0]];s[1] = sbox[s[1]];s[2] = sbox[s[2]];
  s[3] = sbox[s[3]];s[4] = sbox[s[4]];s[5] = sbox[s[5]];
  s[6] = sbox[s[6]];s[7] = sbox[s[7]];s[8] = sbox[s[8]];
  s[9] = sbox[s[9]];s[10] = sbox[s[10]];s[11] = sbox[s[11]];
  s[12] = sbox[s[12]];s[13] = sbox[s[13]];s[14] = sbox[s[14]];
  s[15] = sbox[s[15]];*/
}

void mixColumns(__private uchar* arr){
  
  #pragma unroll
  for (int i = 0; i < 4; ++i) {

    __private uchar *r = arr + 4*i;

    uchar a[4];
    uchar b[4];

    uchar h;

    #pragma unroll
    for(int c = 0; c < 4; ++c) {
      a[c] = r[c];
      h = (uchar)((signed char)r[c] >> 7);
      b[c] = r[c] << 1;
      b[c] ^= 0x1B & h;
    }

    r[0] = b[0] ^ a[3] ^ a[2] ^ b[1] ^ a[1];
    r[1] = b[1] ^ a[0] ^ a[3] ^ b[2] ^ a[2];
    r[2] = b[2] ^ a[1] ^ a[0] ^ b[3] ^ a[3];
    r[3] = b[3] ^ a[2] ^ a[1] ^ b[0] ^ a[0];
  }
}

void shiftRows(__private uchar* s){
  
  uchar t;
  t = s[ 1]; s[ 1] = s[ 5]; s[ 5] = s[ 9]; s[ 9] = s[13]; s[13] = t;
  t = s[ 2]; s[ 2] = s[10]; s[10] = t; t = s[ 6]; s[ 6] = s[14]; s[14] = t;
  t = s[ 7]; s[ 7] = s[ 3]; s[ 3] = s[15]; s[15] = s[11]; s[11] = t;
}

void addRoundKey(__private uchar* state,__global uint* w, __private int i){
  
  uint* s = (uint*) state;

  #pragma unroll
  for (int j = 0; j < Nb; ++j) {
    s[j] ^= w[i + j];
  }
} 

void encrypt(__private uchar state[BLOCK_SIZE], __global uint *w, __private uchar out[BLOCK_SIZE]){
  
  addRoundKey(state, w, 0);

  #pragma unroll
  for (int round = 1; round < Nr; ++round) {
    subBytes(state);
    shiftRows(state); 
    mixColumns(state); 
    addRoundKey(state, w, round*Nb);
  }

  subBytes(state);
  shiftRows(state);
  addRoundKey(state, w, Nr*Nb);

  #pragma unroll
  for (int i = 0; i < 4*Nb; ++i) {
    out[i] = state[i];
  }
}

__kernel void aesCipher(__global uchar* in, __global uint *w, __global uchar* out){

  __private int gid;
  gid = get_global_id(0); 

  __private uchar state[4*Nb];
  
  #pragma unroll
  for (int i = 0; i < 4*Nb; ++i) {
    int offset = gid * BLOCK_SIZE + i;
    state[i] = in[offset];
  }

  /* call encrypt and get output */
  __private uchar outCipher[BLOCK_SIZE];
  encrypt(state, w, outCipher);
  
  #pragma unroll
  for(int i = 0; i < BLOCK_SIZE; i++) {
    int offset = gid * BLOCK_SIZE + i;
    out[offset] = outCipher[i];
  } 
}



__kernel void aesCipherCtr(__global uchar* in, __global uint *w, __global uchar* out){

  __private int gid;
  gid = get_global_id(0);

  /* Create input for aesCipher */
  __private uchar counter[16];
  
  /* -- initialize counter -- */
  #pragma unroll
  for(int k = 0; k < 8; k++){
    counter[k] = nonce[k];
  }

  __private uchar countBytes[8]; 
  //initialize counter
  countBytes[0] = (char)0;
  countBytes[1] = (char)0;
  countBytes[2] = (char)0;
  countBytes[3] = (char)0;
  countBytes[4] = (char)0;
  countBytes[5] = (char)0;
  countBytes[6] = (char)0;
  countBytes[7] = (char)0;
  int n = 8, c = gid;

  /* increment the counter by gid */
  do {
    --n;
    c += countBytes[n];
    countBytes[n] = (char)c;
    c >>= 8;
  } while (n);
  #pragma unroll
  for(int k = 8; k < 16; k++){
    counter[k] = countBytes[k-8];
  }  

  /* call encrypt and get output */
  __private uchar outCipher[BLOCK_SIZE];
  encrypt(counter, w, outCipher);


  /* final xor */
  #pragma unroll
  for (int i = 0; i < 4*Nb; ++i) {
    int offset = gid * BLOCK_SIZE + i;
    out[offset] = outCipher[i] ^ in[offset];
  }
}
