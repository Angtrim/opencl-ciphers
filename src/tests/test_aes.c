
#include <stdio.h>
#include <inttypes.h>
#include <stdlib.h>

#include "test_utils.h"

#include "test_aes.h"

#include "../cipher_utils.h"

#define MAX_LOCAL_SIZE 64


static uint32_t aes128Key[16] = {0x6ede8bab, 0xdad63ffa, 0x796c767d, 0x92e52e38};
static uint32_t aes192Key[24] = {0xded642b8, 0x2daf73f0, 0x7a0f5caa, 0x554dc9c6, 0x21437628, 0x99c92ca2};
static uint32_t aes256Key[32] = {0x63375233, 0xca899f06, 0x2f868c91, 0x8eb2ccc1, 0x36eb5001, 0x58c75cbb, 0x2ee60020, 0x3286d86f};

uint8_t aesPlaintext[128]= {
	 };
uint8_t aes128Ciphertext[128]= {
	 };
uint8_t aes192Ciphertext[128] = {
	 };
 uint8_t aes256Ciphertext[128] = { 
	 };


// to save ciphertext to file (uint8_t)
static void writeOutputToFile(char* outFileName,uint8_t* output, long lenght){
	FILE* fp = fopen(outFileName, "wb");
	if (!fp) {
		fprintf(stderr, "Failed to load file.\n");
		exit(1);
	}
	fwrite(output, sizeof(char), lenght, fp);
	fclose(fp);
}

// TEST FOR 128

int testAes128(cl_device_id* device_id){


	int success = 1;

	uint8_t* aesCiphertext = (uint8_t*)malloc(128*sizeof(uint8_t));
	writeOutputToFile("aes_plaintext", aesPlaintext, 128);

	aes128Encrypt("aes_plaintext", aes128Key, aesCiphertext, 1, device_id);
	if (memcmp(aesCiphertext, aes128Ciphertext, 128) != 0) {
		success = 0;
	}
	free(aesCiphertext);
	remove("aes_plaintext");
	return success;
}


int testAes128Ctr(cl_device_id* device_id){
	int success = 1;
	struct FileInfo fileInfo = getFileBytes("ctr_test");
	long dim = fileInfo.lenght;
	uint8_t* aesCiphertext = (uint8_t*)malloc(dim*sizeof(uint8_t));
	uint8_t* aesPlaintext = (uint8_t*)malloc(dim*sizeof(uint8_t));

	aes128CtrEncrypt("ctr_test", aes128Key, aesCiphertext, 2,device_id);
	writeOutputToFile("aes_ciphertext", aesCiphertext, fileInfo.lenght);
	aes128CtrDecrypt("aes_ciphertext", aes128Key, aesPlaintext, 2, device_id);
	
	if (memcmp(aesPlaintext, fileInfo.filePointer, fileInfo.lenght) != 0) {
		success = 0;
	} 

	free(aesCiphertext);
	free(aesPlaintext);
	free(fileInfo.filePointer);
	remove("aes_ciphertext");
	return success;
}




// Test 192

int testAes192(cl_device_id* device_id){
	int success = 1;
	uint8_t* aesCiphertext = (uint8_t*)malloc(128*sizeof(uint8_t));
	writeOutputToFile("aes_plaintext", aesPlaintext, 128);

	aes192Encrypt("aes_plaintext", aes192Key, aesCiphertext, 1,device_id);
	if (memcmp(aesCiphertext, aes192Ciphertext, 128) != 0) {
		success = 0;
	}
	free(aesCiphertext);
	remove("aes_plaintext");
	return success;
}



int testAes192Ctr(cl_device_id* device_id){
	int success = 1;
	struct FileInfo fileInfo = getFileBytes("ctr_test");
	long dim = fileInfo.lenght;
	uint8_t* aesCiphertext = (uint8_t*)malloc(dim*sizeof(uint8_t));
	uint8_t* aesPlaintext = (uint8_t*)malloc(dim*sizeof(uint8_t));

	aes192CtrEncrypt("ctr_test", aes192Key, aesCiphertext, 2, device_id);
	writeOutputToFile("aes_ciphertext", aesCiphertext, fileInfo.lenght);
	aes192CtrDecrypt("aes_ciphertext", aes192Key, aesPlaintext, 2, device_id);
	
	if (memcmp(aesPlaintext, fileInfo.filePointer, fileInfo.lenght) != 0) {
		success = 0;
	} 

	free(aesCiphertext);
	free(aesPlaintext);
	free(fileInfo.filePointer);
	remove("aes_ciphertext");
	return success;
}




// Test 256
int testAes256(cl_device_id* device_id){
	int success = 1;
	uint8_t* aesCiphertext = (uint8_t*)malloc(128*sizeof(uint8_t));
	writeOutputToFile("aes_plaintext", aesPlaintext, 128);
	aes256Encrypt("aes_plaintext", aes256Key, aesCiphertext, 1, device_id);

	if (memcmp(aesCiphertext, aes256Ciphertext, 128) != 0) {
		success = 0;
	}
	free(aesCiphertext);
	remove("aes_plaintext");
	return success;
}


int testAes256Ctr(cl_device_id* device_id){
	int success = 1;
	struct FileInfo fileInfo = getFileBytes("ctr_test");
	long dim = fileInfo.lenght;
	uint8_t* aesCiphertext = (uint8_t*)malloc(dim*sizeof(uint8_t));
	uint8_t* aesPlaintext = (uint8_t*)malloc(dim*sizeof(uint8_t));

	aes256CtrEncrypt("ctr_test", aes256Key, aesCiphertext, 2, device_id);
	writeOutputToFile("aes_ciphertext", aesCiphertext, fileInfo.lenght);
	aes256CtrDecrypt("aes_ciphertext", aes256Key, aesPlaintext, 2, device_id);
	
	if (memcmp(aesPlaintext, fileInfo.filePointer, fileInfo.lenght) != 0) {
		success = 0;
	} 

	free(aesCiphertext);
	free(aesPlaintext);
	free(fileInfo.filePointer);
	remove("aes_ciphertext");
	return success;
}



int test128(cl_device_id* device_id){
	int result = 1;
	log("--- --- Starting AES128 tests");
	
	log("--- Test AES 128 starting");
	if(testAes128(device_id) == 1){
		log("--- Test AES 128  passed");
	}else{
		log("--- Test AES 128 FAILED!");
		result = 0;
	}


	log("--- Test AES 128 CTR  starting");
	if(testAes128Ctr(device_id) == 1){
		log("--- Test AES 128 CTR passed");
	}else{
		log("--- Test AES 128 CTR FAILED!");
		result = 0;
	}


	if(result != 0){
		log("--- --- All Aes 128 test passed");
	}else{
		log("--- --- Some AES 128 Test FAILED");
	}
	return result;

}

int test192(cl_device_id* device_id){
	int result = 1;
	log("--- --- Starting AES192 tests");
	
	log("--- Test AES 192 starting");
	if(testAes192(device_id) == 1){
		log("--- Test AES 192  passed");
	}else{
		log("--- Test AES 192 FAILED!");
		result = 0;
	}


	log("--- Test AES 192 CTR CPU starting");
	if(testAes192Ctr(device_id) == 1){
		log("--- Test AES 192 CTR  passed");
	}else{
		log("--- Test AES 192 CTR  FAILED!");
		result = 0;
	}


	if(result != 0){
		log("--- --- All Aes 192 test passed");
	}else{
		log("--- --- Some AES 192 Test FAILED");
	}

	return result;
	
}

int test256(cl_device_id* device_id){
	int result = 1;
	log("--- --- Starting AES256 tests");
	
	log("--- Test AES 256 starting");
	if(testAes256(device_id) == 1){
		log("--- Test AES 256  passed");
	}else{
		log("--- Test AES 256 FAILED!");
		result = 0;
	}


	log("--- Test AES 256 CTR  starting");
	if(testAes256Ctr(device_id) == 1){
		log("--- Test AES 256 CTR  passed");
	}else{
		log("--- Test AES 256 CTR  FAILED!");
		result = 0;
	}

	if(result != 0){
		log("--- --- All Aes 256 test passed");
	}else{
		log("--- --- Some AES 256 Test FAILED");
	}

	return result;
	
}

int testAESAll(cl_device_id* device_id){

	int t128 = test128(device_id);
	int t192 = test192(device_id);
	int t256 = test256(device_id);	
	int result = t128&&t192&&t256;
	if(result){
		log("--- --- --- ALL AES TEST PASSED");
	}else{
		log("--- --- --- TEST AES FAILED");
	}
	return result;

}








