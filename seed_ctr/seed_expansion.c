...
//OLD SBOX mode use old 8bit sboxes (slower but smaller memory footprint)
...
uint32_t G_OLD(uint32_t X) {
	uint8_t X0 = (X >> 8*0) & 0xff;
	uint8_t X1 = (X >> 8*1) & 0xff;
	uint8_t X2 = (X >> 8*2) & 0xff;
	uint8_t X3 = (X >> 8*3) & 0xff;

	uint32_t Z0 = (S0[X0] & m0) ^ (S1[X1] & m1) ^ (S0[X2] & m2) ^ (S1[X3] & m3);
	uint32_t Z1 = (S0[X0] & m1) ^ (S1[X1] & m2) ^ (S0[X2] & m3) ^ (S1[X3] & m0);
	uint32_t Z2 = (S0[X0] & m2) ^ (S1[X1] & m3) ^ (S0[X2] & m0) ^ (S1[X3] & m1);
	uint32_t Z3 = (S0[X0] & m3) ^ (S1[X1] & m0) ^ (S0[X2] & m1) ^ (S1[X3] & m2);
...
uint32_t G(uint32_t X) {
	...
	uint32_t Z = SS0[X0] ^ SS1[X1] ^ SS2[X2] ^ SS3[X3];
...
// into four 32-bit blocks (Key0, Key1, Key2, Key3).
//
// Ki0 is the most significant 32 bits of Ki, and Ki1 is the least
...
void seed_old_expandkey(uint32_t *Key, uint32_t* K) {
  ...
  #pragma unroll
	for(int i = 0; i < 16; ++i) {
		K[2*i] = G_OLD(Key0 + Key2 - KC[i]);
		K[2*i+1] = G_OLD(Key1 - Key3 + KC[i]);

    // Condition reversed because index starts at 0 vs. 1
		if (is_even(i)) {
      // Key0 || Key1 = (Key0 || Key1) >> 8
			uint32_t K0 = Key0 >> 8 | Key1 << 24;
			uint32_t K1 = Key1 >> 8 | Key0 << 24;
			Key0 = K0;
			Key1 = K1;
		} else {
      // Key2 || Key3 = (Key2 || Key3) << 8
			uint32_t K2 = Key3 >> 24 | Key2 << 8;
			uint32_t K3 = Key2 >> 24 | Key3 << 8;
			Key2 = K2;
			Key3 = K3;
		}
	}
...
void seed_expandkey(uint32_t *Key, uint32_t* K) {
  ...
  #pragma unroll
	for(int i = 0; i < 16; ++i) {
		K[2*i] = G(Key0 + Key2 - KC[i]);
		K[2*i+1] = G(Key1 - Key3 + KC[i]);
